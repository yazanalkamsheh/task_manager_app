...
class UpdateTaskScreen extends StatefulWidget {
  ...
  const UpdateTaskScreen({super.key, required this.todoEntity});
...
class _UpdateTaskScreenState extends State<UpdateTaskScreen> {
  ...
  @override
  void initState() {
    todoController = TextEditingController(text: widget.todoEntity.todo);
    completed = ValueNotifier(widget.todoEntity.completed);
        ...
        child: BlocListener<TodoBloc, BaseState<TodoEntity>>(
          ...
          child: Scaffold(
            appBar: AppBar(
              ...
              title: Text(
                LocaleKeys.updateTask.tr(context: context),
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (!state.isLoading)
                          AppButton(
                            lable: LocaleKeys.update.tr(context: context),
                            onTap: () => _updatePressed(context, state),
                          ),
                      ],
                    ),
                  ),
  ...
  TodoParameters get todoParameters => TodoParameters(widget.todoEntity.id,
      todoController.text, completed.value, widget.todoEntity.userId);

  void _updatePressed(BuildContext context, BaseState<void> state) {
    if (!(formKey.currentState!.validate()) || state.isLoading) return;

    context.read<TodoBloc>().add(UpdateTodoEvent(todoParameters));
  }

  void _updateListener(BuildContext context, BaseState<TodoEntity> state) {
    ...
    if (state.isSuccess) {
      showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text(LocaleKeys.updateResult.tr(context: context), style: Theme.of(context).textTheme.bodySmall),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
